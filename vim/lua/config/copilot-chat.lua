...
chat.setup({
  ...
  contexts = {
    vectorspace = {
      description = "Semantic search through workspace using vector embeddings.",
      schema = {
        type = "object",
        required = { "query" },
        properties = {
  ...
  providers = {
    ...
    gemini = {
      ...
      get_models = function(headers)
        local response, err = cutils.curl_get(
        ...
        return vim.tbl_map(function(model)
          ...
          return { id = id, name = id, streaming = true, tools = true }
        end, response.body.data)
      end,
      embed = function(inputs, headers)
        local response, err = cutils.curl_post(
          "https://generativelanguage.googleapis.com/v1beta/openai/embeddings",
          {
            headers = headers,
            json_request = true,
            json_response = true,
            body = { input = inputs, model = "text-embedding-004" },
          }
        )
        if err then
          error(err)
